...
start = time.time()
video = myclip
print("Duration of input is "+str(video.duration))
...
def score_recog(start,end):
    ...
    return hs
if len(last) is 0:
	print("There is no highlights present in the video")
else:
...
if len(clips)>0:
	...
	print("Duration of output is "+str(VideoFileClip(source.replace(".mp4","_highlights.mp4")).audio.duration))
else:
	print("There is no Highlights present in the video")
